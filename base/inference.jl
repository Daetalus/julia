...
struct AllocOptContext
    ...
    function AllocOptContext(infomap::ValueInfoMap, sv::InferenceState)
        ...
        return new(infomap, sv, todo, ObjectIdDict(), ObjectIdDict(),
                   ObjectIdDict(), ObjectIdDict(), ObjectIdDict())
    end
end
...
function split_struct_alloc!(ctx::AllocOptContext, info, key)
    ...
    if ndef == 1
        ...
        @assert min_nf == max_nf
        if !isempty(ctx.undef_fld) && has_setfield_undef
            flag_vars = Vector{SlotNumber}(min_nf)
        end
        vars = Vector{Any}(min_nf)
        ...
        for i in 1:min_nf
            ...
            field_typ = has_def ? widenconst(exprtype(si.defs[i], ctx.sv.src,
                                                      ctx.sv.mod)) : Union{}
            ...
            if !has_def
                ...
                add_def(ctx.infomap, flag_slot,
                        ValueDef(newvar_flag, def_exprs, length(def_exprs)))
                continue
...
function verify_seen_info(ctx::AllocOptContext, seen, def_or_use)
    # if !haskey(seen, def_or_use)
    #     println(def_or_use)
    #     println(ctx.sv.src.code)
    #     println(ctx.infomap)
    # end
